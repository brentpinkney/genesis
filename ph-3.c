//
// Pre-history 3: add inital eval, only null, integer, symbol, !, ?."
//
#include <stdio.h>
#include <stdlib.h>
#include <sys/mman.h>
...
static cell * eval_list( cell * lst, cell * env )
{
	cell * op = lst->car;
	dprintf( "eval-list: op = %016lx\n", op->header );
	switch( symbol_value( op ) )
	{
		case '!':
		{
			cell * k = lst->cdr->car;
			cell * v = lst->cdr->cdr->car;

			cell * assoc = assq( k, env->cdr, env );
			if( is_null( assoc, env ) != env->car )
			{
				assoc = cons( k, v, env );
				env->cdr = cons( assoc, env->cdr, env );
			}
			else
			{
				set_cdr( assoc, v, env );
			}
			return cons( assoc, env, env );
			break;
		}
		case '?':
		{
			cell * tst = lst->cdr->car;
			cell * con = lst->cdr->cdr->car;
			cell * alt = lst->cdr->cdr->cdr->car;

			cell * ans = eval( tst, env );
			tst = ans->car;
			env = ans->cdr;
			if( is_null( tst, env ) != env->car )
			{
				cell * ans = eval( alt, env );
				return cons( ans->car, env, env );
			}
			else
			{
				cell * ans = eval( con, env );
				return cons( ans->car, env, env );
			}
			break;
		}
	...
	dprintf( "eval-list: default\n" );
	return cons( lst, env, env );
}
...
static cell * eval( cell * exp, cell * env )
{
	switch( cell_type( exp ) )
	{
		...
		case TYPE_SYMBOL:
		{
			cell * assoc = assq( exp, env->cdr, env );
			if( is_tuple( assoc, env ) != env->car )
			{
				return cons( assoc->cdr, env, env );
			}
			else
			{
				return cons( env->car, env, env );
			}
			break;
		}
		case TYPE_INTEGER:
...
static cell * repl( cell * env )
{
	...
	cell * ans = eval( read( env ), env );
	env = print( ans->car, ans->cdr );
